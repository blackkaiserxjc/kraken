...
#define KRLOG_SEV_IMPL(logger, level, file, line, tag) \
    BOOST_LOG_STREAM_WITH_PARAMS((logger),             \
        ((logger).set_attr_value("File", static_cast<std::string>(file)))((logger).set_attr_value("Line", static_cast<int>(line)))((logger).set_attr_value("Tag", static_cast<std::string>(tag)))(boost::log::keywords::severity = (level)))
